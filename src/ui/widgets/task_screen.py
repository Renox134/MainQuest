...
class TaskScreen(MDScreen):
    ...
    def update_widgets(self) -> None:
        ...
        self.start_time_dialog.bind(on_ok=self.confirm_start_time_selection)
    ...
    def open_start_time_selector(self):
    ...
    def confirm_start_time_selection(self, start_time_dialog: MDTimePickerDialVertical) -> None:
        hour: int = int(start_time_dialog.hour)
        if start_time_dialog.am_pm == "pm":
            hour = (hour + 12) % 24 if hour > 0 else 0
        minute: int = int(start_time_dialog.minute)
